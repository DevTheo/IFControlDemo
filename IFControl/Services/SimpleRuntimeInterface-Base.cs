...
{
    public sealed partial class SimpleRuntimeInterface : IScreenSetup, ISimpleTextOnlySupport, IComplexTextSupport, IBasicImageSupport, IAnimationSupport, IBasicSoundSupport, IFileStorageSupport, IOtherInputSupport
    {
        readonly BaseImplemention baseImpl = new BaseImplemention();
        private List<FrameworkElement> controls { get { return baseImpl.controls; } }
        protected bool actionOnControl(int ctlNum, Action<ConsoleControl> action)
        {
            if (ctlNum >= baseImpl.controls.Count && ctlNum >= 0)
                return false;
            var ctl = baseImpl.controls[ctlNum];
            if (ctl is ConsoleControl)
            {
                var content = (ConsoleControl)ctl;
                action(content);
                return true;
            }
            return false;
        ...
        {
            return GetLineAsyncTask().AsAsyncOperation();
        }
