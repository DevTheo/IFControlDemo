...
namespace IFCore.Services
{
    ...
    public sealed partial class SimpleRuntimeInterface
    {
        ...
        public bool IsGridText(int controlNum)
        {
            ...
            return ctl is IIFTextGridControl;
        }
        ...
        public TupleIntInt GetRowsColumns(int controlNum)
        {
            ...
            var ctl = (IIFTextGridControl)controls[controlNum];
        ...
        public int CreateRegion(WindowType winType, string qlocation, int width, int height, bool scrollable, int rows, int cols)
        {
            ...
            if (winType == WindowType.MainTextRegion)
            {
                ...
                applyQuadrant(_ui as FrameworkElement, qlocation, -1, -1);
                GotMainWin = true;
                controls.Add(_ui as FrameworkElement);
                return controls.Count - 1;
            }
            if (winType == WindowType.TextRegion)
            {
                var ctl = CreateConsoleTextControl(scrollable, false);
                applyQuadrant(ctl as FrameworkElement, qlocation, width, height);
                controls.Add(ctl as FrameworkElement);
                return controls.Count - 1;
            }
        ...
        private IIFTextControl CreateConsoleTextControl(bool scrollable, bool allowInput)
        {
            return
                (IOC.ControlCreator != null) ?
                IOC.ControlCreator.CreateTextControl(scrollable, allowInput) :
                null;
        }
        private IIFTextGridControl CreateConsoleGridTextControl(bool scrollable, bool allowInput)
        {
            return
                (IOC.ControlCreator != null) ?
                IOC.ControlCreator.CreateGridTextControl(scrollable, allowInput) :
                null;
        }
