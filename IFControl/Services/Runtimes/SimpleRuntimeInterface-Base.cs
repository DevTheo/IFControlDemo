...
{
    public sealed partial class SimpleRuntimeInterface : IIFRuntime
    {
        ...
        protected bool actionOnControl(int ctlNum, Action<FrameworkElement> action)
        {
            ...
            {
                var content = (FrameworkElement) ctl;
                action(content);
                return true;
            }
            return false;
        ...
        protected bool actionOnIFControl(int ctlNum, Action<IIFControl> action)
        {
            if (ctlNum >= baseImpl.controls.Count && ctlNum >= 0)
                return false;
            var ctl = baseImpl.controls[ctlNum];
            if (ctl is IIFControl)
            {
                var content = (IIFControl) ctl;
                action(content);
                return true;
            }
            return false;
