    ...
    internal class DebugState {
        ...
        public static void StartState(String stateFileToLoad) {
            if (stateFileToLoad != null) {
                ...
                var fileId = os_.fileIO.OpenLocalFileForReadAsync(stateFileToLoad).GetResults();

                try
                {
                    var good = new StreamReader(os_.fileIO.GetStreamReaderAsync(fileId).GetResults().AsStream());
